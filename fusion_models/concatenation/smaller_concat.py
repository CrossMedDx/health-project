...
from transformers import BertModel, get_linear_schedule_with_warmup, ViTModel
...
def get_loader(ds, bs, shuffle=False):
    return DataLoader(
        ds, batch_size=bs, shuffle=shuffle, collate_fn=fusion_collate_fn, num_workers=4
    )
...
results_file = "smaller_concatenation_fusion_results.json"
...
def execute_hyperparameter_combo(combo):
    ...
    # check if name exists in results file
    ...
    for epoch in range(1, combo["num_epochs"] + 1):
        ...
        print(
            f"Epoch {epoch}/{combo['num_epochs']} - Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}"
        )
